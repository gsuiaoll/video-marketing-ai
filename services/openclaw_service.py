...
def _call_ai(user_name: str, content: str) -> dict:
    """通过 OpenClaw Agent 做智能客服回复"""
    user_prompt = f"""用户名称：{user_name}
用户消息：{content}

请以小龙虾客服的身份回复这条消息。请严格按 JSON 格式输出，不要输出其他内容：
{{"reply": "回复内容", "confidence": 0.85, "intent": "意图类别", "needs_human": false}}"""

    try:
        from services.ai_script import call_ai
        result = call_ai(user_prompt, system=CS_SYSTEM_PROMPT, temperature=0.7)
        if result:
            # 尝试提取 JSON
            import re
            ...
            if json_match:
                ...
                return {
                    "reply": data.get("reply", "感谢您的咨询，请问有什么可以帮助您的？"),
